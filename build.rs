    ...
    for entry in fs::read_dir(src)? {
        ...
        if path.is_dir() {
            ...
            if let Some(file_extension) = path.extension() {
                if file_extension.to_str().unwrap().to_ascii_lowercase() == extension
                    || extension == "*"
                {
...
fn main() {
    ...
    let dest_test_dir = Path::new(&out_dir)
        .join("..")
        .join("..")
        .join("..")
        .join("deps");
    ...
    if let "windows" = platform.as_str() {
        ...
        copy_all_with_extension(&lib_dir, &dest_dir.to_str().unwrap(), "dll")
            .expect("Unable to copy native libraries");

        // Thanks https://github.com/rust-lang/cargo/issues/4044
        copy_all_with_extension(&lib_dir, &dest_test_dir.to_str().unwrap(), "dll")
            .expect("Unable to copy native libraries");

        println!(
            "{}",
            format!("cargo:rustc-link-search={}", lib_dir.to_str().unwrap())
        );
    ...
    for (folder, extension) in &[
        ...
        if path.exists() {
            copy_all_with_extension(&path, &dest_dir.join(folder).to_str().unwrap(), extension)
                .unwrap_or_else(|_| panic!("Unable to copy {folder}"));
        }
    }
}
