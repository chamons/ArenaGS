...
use crate::after_image::{load_image, RenderContext, RenderContextHolder};

use crate::atlas::{get_exe_folder, BoxResult, EasyPath};

pub struct IconLoader {
    images: HashMap<String, String>,
    render_context_holder: RenderContextHolder,
}

impl IconLoader {
    pub fn init(render_context_holder: RenderContextHolder, subfolder: &str) -> BoxResult<IconLoader> {
        ...
        Ok(IconLoader { images, render_context_holder })
    }

    pub fn get(&self, name: &str) -> BoxResult<Texture> {
        let name = name.to_ascii_lowercase();
        if let Some(path) = self.images.get(&name) {
            let render_context = self.render_context_holder.borrow();
