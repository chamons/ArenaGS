...
pub fn apply_healing_to_character(ecs: &mut World, amount: Strength, target: Entity) {
    let healing_total = {
        ...
        let mut defenses = ecs.write_storage::<DefenseComponent>();
...
fn apply_damage_core(ecs: &mut World, damage: Damage, target: Entity, source_position: Option<Point>) {
    ...
    let rolled_damage = {
        let mut defenses = ecs.write_storage::<DefenseComponent>();
        let defenses = &mut defenses.grab_mut(target).defenses;
...
mod tests {
    ...
    fn triple_shot_applies_armor_each_time() {
        ...
        ecs.write_storage::<DefenseComponent>().grab_mut(target).defenses.armor = 6;
    ...
    fn regen_adds_health_over_turns() {
        ...
        ecs.write_storage::<DefenseComponent>().grab_mut(player).defenses.health = 5;
